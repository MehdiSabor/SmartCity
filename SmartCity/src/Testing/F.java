...
import Testing.WM.SwingAction;
import Testing.WM.SwingAction_1;
...
public class F extends JPanel {
	 ...
	 * @throws IOException 
	 */
	public F(Resident R) throws IOException {
setBounds(0,0,593,498);
		...
		JTextArea textArea = new JTextArea("Fridge");
		textArea.setBounds(209, 155, 164, 22);
		textArea.setEditable(false);
		textArea.setOpaque(false);
		add(textArea);
		
		JTextArea textArea_1 = new JTextArea("Avaibility: "+Boolean.toString(R.getAppart().Appliances[i].isAvailable));
		textArea_1.setBounds(122, 187, 154, 22);
		add(textArea_1);
		
		
		JTextArea textArea_2 = new JTextArea("Working: "+ Boolean.toString(R.getAppart().Appliances[i].isWorking));
	...
	public class SwingAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			user.getAppart().Appliances[i].setAvailable(!(a.Appliances[i].isAvailable));
			try {
				Arr.fileupdate();
				System.out.println((a.Appliances[i].isWorking));
			} catch (IOException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
			System.out.println((a.Appliances[i].isAvailable));
		}
	}
	public class SwingAction_1 extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			user.getAppart().Appliances[i].setWorking(!(a.Appliances[i].isWorking));
			try {
				Arr.fileupdate();
				System.out.println((a.Appliances[i].isWorking));
			} catch (IOException e1) {
