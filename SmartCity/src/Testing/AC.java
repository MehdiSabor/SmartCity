...
public class AC extends JPanel  {
	 ...
	 * @throws IOException 
	 */
	public AC(Resident R) throws IOException {
setBounds(0,0,593,498);
		...
		JTextArea textArea = new JTextArea("AC");
		textArea.setBounds(209, 155, 164, 22);
		textArea.setEditable(false);
		textArea.setOpaque(false);
		add(textArea);
		
		JTextArea textArea_1 = new JTextArea("Avaibility: "+Boolean.toString(R.getAppart().Appliances[2].isAvailable));
		textArea_1.setBounds(122, 187, 154, 22);
		add(textArea_1);
		
		
		JTextArea textArea_2 = new JTextArea("Working: "+ Boolean.toString(R.getAppart().Appliances[2].isWorking));
	...
	private class SwingAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			user.getAppart().Appliances[2].setAvailable(!(a.Appliances[2].isAvailable));
			try {
				Arr.fileupdate();
				System.out.println((a.Appliances[2].isWorking));
			} catch (IOException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
			System.out.println((a.Appliances[2].isAvailable));
		}
	}
	private class SwingAction_1 extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			user.getAppart().Appliances[2].setWorking(!(a.Appliances[2].isWorking));
			try {
				Arr.fileupdate();
				System.out.println((a.Appliances[2].isWorking));
			} catch (IOException e1) {
